...
@PrototypeScope
@SpringView(name = ShowSubjectView.NAME)
public class ShowSubjectViewImpl extends Window implements ShowSubjectView, Serializable {
    ...
    private final Label lbtitle = new Label("对象详情");
    ...
    @Override
    public void initView(ShowSubjectPresenter presenter) {
        center();
    ...
    public void bind(ShowSubjectPresenter presenter) {
        ...
        btn2.addClickListener(e -> {
            try {
                ...
                if (!presenter.checkIdNo(command.getIdNo())) {
                    Notification.show("身份证已被注册！");
                    return;
                }

                if (!presenter.checkPhone(command.getPhone())) {
                    Notification.show("电话号码已被注册！");
                    return;
                }
    ...
    public void addStyle() {
        this.mainvly.addStyleName("ShowSubjectViewImpl");
    ...
    @Override
    public void refreshValue(SubjectModel model) {
        name.setValue(model.getPersonInfo().getSubjectName());
        subjectType.select(new IdAndName(model.getSubjectInfo().getSubjectTypeId(), model.getSubjectInfo().getSubjectTypeName()));
        idNo.setValue(model.getPersonInfo().getIdcardno());
        phone.setValue(model.getContactPhone());
        overDraft.setValue(model.getSubjectInfo().getOverdraft().toString());
        address.setValue(model.getAddress());
        sendType.select(new IdAndName(model.getSubjectInfo().getDeliveType(), model.getSubjectInfo().getDeliveType().equals("SITE") ? "助餐点" : "到户上门"));
        mealSite.select(new IdAndName(model.getSubjectInfo().getDeliveSiteId(), model.getSubjectInfo().getDeliveSiteName()));
        desc.setValue(model.getSubjectInfo().getSubjectDesc());
        jinjiName.setValue(model.getSubjectInfo().getRelationName());
        jinjiPhone.setValue(model.getSubjectInfo().getRelationPhone());
        wnjinji.select(new IdAndName(model.getFamilyRelationId(), model.getFamilyRelationName()));
        model.getNxftSubjectDiseaseSet().forEach(e -> {
            this.diseases.select(new IdAndName(e.getDiseaseId(), e.getDiseaseName()));
            if (e.getDiseaseId().equals("99")) {
                jbtf.setValue(e.getDiseaseName());
            }
        });
        wrahuji.select(model.getIsNative() ? new IdAndName("1", "本镇") : new IdAndName("2", "非本镇"));
        model.getNxftSubjectLivingCondiSet().forEach(e -> {
            livingConditions.select(new IdAndName(e.getLivingCondiId(), e.getLivingCondiName()));
        });

        wrafuzhu.select(model.getIsGovHelp() ? new IdAndName("1", "是") : new IdAndName("2", "否"));
        wchfuzhu.select(new IdAndName(model.getGovHelpId(), model.getGovHelpName()));
        wraguom.select(model.getHaveFoodAllergy() ? new IdAndName("1", "是") : new IdAndName("2", "否"));
        tfguom.setValue(model.getFoodAllergy() == null ? "" : model.getFoodAllergy());
        model.getNxftSubjectFoodProhibitSet().forEach(e -> {
            wchjinji.select(new IdAndName(e.getFoodProhibitId(), e.getFoodProhibitName()));
            if (e.getFoodProhibitId().equals("99")) {
                jjtf.setValue(e.getFoodProhibitName());
            }
        });
        lunchBoxNo.setValue(model.getSubjectInfo().getLunchBoxNo() == null ? "" : model.getSubjectInfo().getLunchBoxNo());
        subjectFoodLove.select(new IdAndName(model.getPersonInfo().getSubjectFoodLoveId(), model.getPersonInfo().getSubjectFoodLoveName()));
        isMealHelp.select(model.getPersonInfo().getIsMealHelp() ? new IdAndName("1", "是") : new IdAndName("2", "否"));
    }
