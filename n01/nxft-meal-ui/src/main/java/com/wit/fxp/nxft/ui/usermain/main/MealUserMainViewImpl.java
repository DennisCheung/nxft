...
@PrototypeScope
@SpringView(name = MealViewName.MEAL_MAIN_VIEW_NAME)
public class MealUserMainViewImpl extends CustomComponent implements MealUserMainView, View, Serializable {
    ...
    public void addStyle() {
        ...
        mainVsp.setHeight(page.getBrowserWindowHeight() - 143 + "px");
        page.addBrowserWindowResizeListener(e -> {
            if (page.getBrowserWindowWidth() > 1230) {
                mainVsp.setWidth(page.getBrowserWindowWidth() - 4 + "px");
            }
            if (page.getBrowserWindowHeight() > 590) {
                mainVsp.setHeight(page.getBrowserWindowHeight() - 140 + "px");
            }
        });
        mainVsp.setSplitPosition(90, Unit.PERCENTAGE);
    ...
    @Override
    public void initChangeSiteMenu(List<TeamViewModel> models, MealUserMainPresenter presenter) {
        barMenu.removeItems();
