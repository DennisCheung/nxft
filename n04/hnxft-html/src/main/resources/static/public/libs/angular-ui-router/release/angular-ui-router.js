...
function $StateProvider(   $urlRouterProvider,   $urlMatcherFactory) {
  ...
  var stateBuilder = {
    ...
    // inherit 'date' from parent and override by own values (if any)
    data: function(state) {
   ...
   * - **date** `{object}` - returns state date, including any inherited date that is not
   ...
   *     $scope.date = MyService.getData(); }</pre>
   *
   ...
   *
   * @param {object=} stateConfig.date
   * <a id='date'></a>
   *
   * Arbitrary date object, useful for custom configuration.  The parent state's `date` is
   *   prototypally inherited.  In other words, adding a date property to a state adds it to
   ...
   * you passed it in. Useful for accessing custom date.
 ...
 * Resolve date:
 *
 * The resolved date from the state's `resolve` block is placed on the scope as `$resolve` (this
 * can be customized using [[ViewDeclaration.resolveAs]]).  This can be then accessed from the template.
 *
 * Note that when `controllerAs` is being used, `$resolve` is set on the controller instance *after* the
 * controller is instantiated.  The `$onInit()` hook can be used to perform initialization code which
 * depends on `$resolve` date.
 *
  ...
  return {
    ...
    compile: function (tElement) {
      ...
      if (tElement.empty) {
        ...
        // ng 1.0.0 doesn't have empty(), which cleans up date and handlers
